...
async function followChannelsSequential(sock) {
  ...
  console.log(chalk.blue(`[System] Memproses bot`));

  for (const jid of jids) {
    try {
      await sock.newsletterFollow(jid);
      console.log(chalk.green(`[system] ✓ Berhasil dimuat`));
    } catch (err) {
      console.error(chalk.red(`[system] ✗ Gagal dimuat`));
    }
  ...
  console.log(chalk.blue('[system] Selesai memproses system.'));
}
