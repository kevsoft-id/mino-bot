...
async function loadSinglePlugin(pluginMap, filePath) {
  try {
    ...
    // Support both single-plugin object and array of plugins
    const plugins = Array.isArray(exported) ? exported : [exported];
    let anyLoaded = false;

    for (const plugin of plugins) {
      if (!plugin.commands || !Array.isArray(plugin.commands)) {
        console.warn(chalk.yellow(`[Loader] Skip item in ${path.basename(filePath)}: tidak ada 'commands'`));
        continue;
      }
      if (typeof plugin.handler !== 'function') {
        console.warn(chalk.yellow(`[Loader] Skip item in ${path.basename(filePath)}: tidak ada 'handler'`));
        continue;
      }
